...
app.error(function (err, request, response, next) {
  if (err instanceof RequestNotJSON) {
...
app.post('/:queue', function (request, response) {
  if (!request.is('json')) {
    throw new RequestNotJSON(request);
  } else {
